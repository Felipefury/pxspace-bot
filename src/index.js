...
function Bot() {
    ...
    that.getChunk = function() {
        ...
        socket.on('ch', (p, d) => {
            ...
            if(chunks_data[p] == false) {
                chunks_data[p] = true
            } else return;
 
            if(firstRun == true) {
                console.log('[BOT] Downloading Chunk: ', (chunk_num-1))
            };
                    ...
                    if (z == chunks.length) {
                        setTimeout(() => {
                            if(firstRun == false) {
                                ...
                                if(!fs.existsSync('./timelapse/' + Template.replace(".png", ""))) {
                                    fs.mkdirSync('./timelapse/' + Template.replace(".png", ""))
                                };
                                imgChunk.write('./timelapse/' + Template.replace(".png", "") + '/' + Date.now() + '.png')
                            } else {
                                firstRun = false
        ...
        if (!Templates[0]) {
            ...
            var quest = [{
                ...
                },
                {
                    ...
                    validate: function(obj) {
                        ...
                        return notobj || "X is not a valid number";
                    },
                    filter: Number
                }, {
                    type: "input",
                    ...
                    validate: function(obj) {
                        var notobj = !isNaN(parseFloat(obj));
                        return notobj || "Y is not a valid number";
                    },
                    filter: Number
                }, {
                    type: "list",
                    ...
                    type: "input",
                    name: "timelapse",
                    message: "do you want to make timelapse? enter the value in seconds, if not just press enter",
                    validate: function(obj) {
                        var notobj = !isNaN(parseFloat(obj));
                        return notobj || "this value is not valid";
                    },
                    filter: Number
                }
            ];
            ...
            inquirer.prompt(quest).then((result) => {
                ...
                jimp.read('./' + Template, function(err, img) {
                    ...
                    console.log("[BOT] Necessary chunks: " + chunks.length)
